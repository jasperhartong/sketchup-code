...
# create places four side-by-side pairs along +X: extended, half-extended, retracted, retracted + smalls on ground.
...
module Timmerman
  module ExtendableBed
    ...
    TOP_OF_SLATS_Z = 250.mm # Upper face of comb slats (+Z).
    ...
    # Same frame positions as Ret; pillows show big on slats and three small cushions on the floor (Z=0).
    GROUP_RETGND_BACK = 'EB_RetGnd_Back'
    GROUP_RETGND_FRONT = 'EB_RetGnd_Front'

    EB_GROUP_NAME_RE = /\AEB_(Ext|Ret|Half|RetGnd)_(Back|Front)\z/
    # Single-pair bed roots (cleared together with EB_Ext_* / EB_Ret_* / EB_Half_* / EB_RetGnd_*).
    ...
    # Big: retracted span + BEAM_NARROW so it sits between head/foot planks; smalls split the remainder equally so extended run sums to LENGTH_EXTENDED:
    # PILLOW_BIG_LENGTH + SMALL_PILLOW_COUNT×PILLOW_SMALL_LENGTH = LENGTH_EXTENDED.
    PILLOW_BIG_LENGTH = LENGTH_RETRACTED + BEAM_NARROW
    SMALL_PILLOW_COUNT = 3
    PILLOW_SMALL_LENGTH = (LENGTH_EXTENDED - LENGTH_RETRACTED - BEAM_NARROW) / SMALL_PILLOW_COUNT

    # Half-extended pair: open span is full extend minus one small cushion width (one segment short of foot plank).
    HALFWAY_FRONT_FOOT_WORLD_Y = LENGTH_EXTENDED - PILLOW_SMALL_LENGTH
    ...
    PILLOW_SMALL_3 = 'EB | pillow | small | 3'
    ...
    # `create` builds four side-by-side pairs (extended, half-extended, retracted, retracted + ground-stored smalls). Stock metre count and cut plan
    ...
    # Big pillow in back-root space; three smalls in front-root space (footward of big). All shifted −Y by BEAM_NARROW
    ...
    def add_pillows_extended_pair(back_group, front_group, layer: nil)
      ...
      [PILLOW_SMALL_1, PILLOW_SMALL_2, PILLOW_SMALL_3].each_with_index do |name, i|
        # +Y head→foot: pillow 1 footward … 3 headward (adjacent to big).
        y_world = y0 + ((SMALL_PILLOW_COUNT - 1 - i) * small_len)
        PillowFactory.add_on_slats(
          ...
          w, small_len,
          layer: layer,
          note: "Extended bed only; small #{i + 1}/#{SMALL_PILLOW_COUNT}; Y order foot→head; equal thirds of (LENGTH_EXTENDED − LENGTH_RETRACTED − BEAM_NARROW)."
        )
      end
    end

    # Retracted pair: big cushion flat; third small on top of big (same z_on_big as couch cushions) in head Y-slot w×t×sm; couch | head and couch | right shifted +Y by t.
    def add_pillows_retracted_pair(back_group, layer: nil)
      ...
      # Same foam volume as flat small (w×sm×t); dy = t fills [y_pillow, y_pillow + t] snug behind shifted couch | head; base Z = top of big pillow.
      PillowFactory.add_prism(
        ...
        w, t, sm,
        layer: layer,
        note: 'Couch mode; small | 3 on big pillow (z_on_big); vertical slab; couch | head shares same seat Z.'
      )

      y_head_cushion = y_pillow + t

      PillowFactory.add_prism(
        back_group.entities,
        PILLOW_COUCH_HEAD,
        Geom::Transformation.translation([0, y_head_cushion, z_on_big]),
        w, t, sm,
        layer: layer,
        note: 'Couch mode; PILLOW_THICKNESS along Y on seat; height PILLOW_SMALL_LENGTH along Z; Y0 = y_pillow + PILLOW_THICKNESS.'
      )

      y_right = y_head_cushion + t
      PillowFactory.add_prism(
        back_group.entities,
        PILLOW_COUCH_RIGHT,
        ...
        t, w, sm,
        layer: layer,
        note: 'Couch mode; +X side; Y0 = y_pillow + 2×PILLOW_THICKNESS; clears both Y bands of couch | head.'
      )
    end

    # Retracted bed (non-extended): big cushion on slats; all small cushions flat on the floor (Z=0).
    # Same prism as extended smalls (w × PILLOW_SMALL_LENGTH × PILLOW_THICKNESS); only Y is shifted so they sit between
    # head legs and mid legs (same mid_y0 as build_back_group). X stays full comb width — foam size is not scaled.
    def add_pillows_retracted_ground_smalls_pair(back_group, layer: nil)
      m = frame_layout_metrics
      w = m[:w]
      z_top_slats = z_slat_top
      y_pillow = BEAM_Y - BEAM_NARROW
      ...
      PillowFactory.add_on_slats(
        ...
        w, PILLOW_BIG_LENGTH,
        layer: layer,
        note: 'Retracted bed; big on slats. Small cushions on ground (stored) — not couch stack.'
      )
      ...
      pad = SLAT_GAP
      y_corridor_lo = y_head + LEG_Y + pad
      y_corridor_hi = mid_y0 - pad
      total_sm = SMALL_PILLOW_COUNT * sm
      ...
      [PILLOW_SMALL_1, PILLOW_SMALL_2, PILLOW_SMALL_3].each_with_index do |name, i|
        y = y_cluster + ((SMALL_PILLOW_COUNT - 1 - i) * sm)
        PillowFactory.add_on_slats(
          back_group.entities,
          name,
          0, y, 0,
          w, sm,
          layer: layer,
          note: "Retracted + stored: small #{i + 1}/#{SMALL_PILLOW_COUNT} on floor (Z=0); same size as extended smalls; Y between head and mid legs."
        )
      end
    end

    # Half-extended: open span = big + two small flats (head-adjacent pair); vertical head at y_pillow only (retracted couch preview uses extra flat + shifted back).
    def add_pillows_halfway_pair(back_group, front_group, layer: nil)
      ...
      PillowFactory.add_prism(
        ...
        w, t, sm,
        layer: layer,
        note: 'Half-extended; vertical back at y_pillow (retracted couch: small | 3 w×t×sm on z_on_big in head slot; back/arm +t in Y).'
      )

      y0 = y_pillow + PILLOW_BIG_LENGTH
      # Head-adjacent smalls only (|3| then |2| toward foot); |1| lies past the halfway foot plane.
      [
        [PILLOW_SMALL_3, y0],
        [PILLOW_SMALL_2, y0 + sm]
      ].each do |name, y_world|
        PillowFactory.add_on_slats(
          front_group.entities,
          name,
          0, y_world - foot_y, z_top_slats,
          w, sm,
          layer: layer,
          note: 'Half-extended; two small flats; same Y slots as extended small | 3 and | 2.'
        )
      end
    end
    ...
    def create(model = Sketchup.active_model)
      ...
      place_pair(
        ...
        offset_x: 3 * (w + gap),
        foot_world_y: RETRACTED_FOOT_WORLD_Y,
        back_name: GROUP_RETGND_BACK,
        front_name: GROUP_RETGND_FRONT
      )
      retgnd_back = find_placement_group_by_name(model, GROUP_RETGND_BACK)
      add_pillows_retracted_ground_smalls_pair(retgnd_back, layer: layer) if retgnd_back
      ...
      puts format('[EB stock] %.3f m extrusion, %d pieces — one bed (%s + %s). Extra preview pairs are not counted.',
